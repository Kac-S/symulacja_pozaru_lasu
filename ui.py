...
def confirm_grid_size():
    ...
    img_data = np.random.random(grid_size)
    img_plt = ax.imshow(img_data, cmap=cmap, vmin=0, vmax=3)
    sim.set_plot(img_plt)
...
def confirm_sim_params():
    ...
    global simulation_percent_parameters
    simulation_percent_parameters = {
...
def run_pressed():
    ...
    sim_params = simulation_percent_parameters | numerical_parameters
    print('sim run')
...
# XY input
validate_numerical_parameters = root.register(validate_input1)

label1 = tk.Label(frame_up_L, text="L:")
label1.pack()
entry1 = tk.Entry(frame_up_C, validate="key", validatecommand=(validate_numerical_parameters, '%d', '%P'))
entry1.insert(0, '45')
entry1.pack()
...
label7.pack()
entry7 = tk.Entry(frame_up_C, validate="key", validatecommand=(validate_numerical_parameters, '%d', '%P'))
entry7.insert(0, '7')
entry7.pack()

validate_parameters = root.register(validate_input2)

label8 = tk.Label(frame_up_L, text="Starting Fires:")
label8.pack()
entry8 = tk.Entry(frame_up_C, validate="key", validatecommand=(validate_parameters, '%d', '%P'))
...
label3.pack()
entry3 = tk.Entry(frame_up_C, validate="key", validatecommand=(validate_parameters, '%d', '%P'))
entry3.insert(0, '0.7')
entry3.pack()

label4 = tk.Label(frame_up_L, text="Spread Probability:")
label4.pack()
entry4 = tk.Entry(frame_up_C, validate="key", validatecommand=(validate_parameters, '%d', '%P'))
entry4.insert(0, '0.2')
entry4.pack()

label6 = tk.Label(frame_up_L, text="Growth chance:")
label6.pack()
entry6 = tk.Entry(frame_up_C, validate="key", validatecommand=(validate_parameters, '%d', '%P'))
entry6.insert(0, '0.05')
entry6.pack()

validate_negative_parameters = root.register(validate_input3)
...
entry5x = tk.Entry(frame_up_C_wind, validate="key",
                   validatecommand=(validate_negative_parameters, '%d', '%P'), width=10)
entry5x.insert(0, '0')
...
reset_button = tk.Button(frame_bottom, text="Reset", command=reset_pressed)
...
while True:
    ...
    if run_flag and (time.time() - tp) > 0.4:
        grid = sim.calculate_fire_spread_probability(grid, 1 - sim_params['probability'], sim_params['burn'],
                                                     sim_params['growth'], sim_params['wind'])
        print(grid)
